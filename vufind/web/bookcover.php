...
function processImageURL($url, $cache = true)
{
    ...
    if ($image = @file_get_contents($url)) {
        ...
        if($_GET['size'] == "small" && $width > 60) {
          $new_width = 60;
          resize_image($new_width, $width, $height, $finalFile);
        }
        elseif($_GET['size'] == "medium" && $width > 150) {
          $new_width = 150;
          resize_image($new_width, $width, $height, $finalFile);
        }
...
// helper function to resize images from GBV cover service
function resize_image($new_width, $width, $height, $finalFile) {
   $new_height = ($new_width / $width) * $height;
   $rs_image = imagecreatetruecolor($new_width, $new_height);
   $image = imagecreatefromjpeg($finalFile);
   imagecopyresampled($rs_image, $image, 0, 0, 0, 0, $new_width, $new_height, $width, $height);
   imagejpeg($rs_image, $finalFile);
}
