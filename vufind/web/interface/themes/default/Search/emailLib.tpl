<div align="left">
  ...
  <div id="popupDetails"> 
  <form action="{$url}/Search/EmailLib" method="post" onSubmit='SendLibEmail(this.elements[&quot;to&quot;].value, 
    ...
    <input type="hidden" name="to" id="to" value="{$supportEmail}"><strong>{$supportEmail}</strong><br>
    <strong><label for="from">{translate text='From'}:</label></strong><br>
    <input type="text" name="from" size="40" id="from" {if $user->email}value="{$user->email}"{/if}><br>
