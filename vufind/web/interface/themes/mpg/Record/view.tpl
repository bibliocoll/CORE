...
{js filename="record-rdg-extra.js"}
{if !empty($addThis)}
...
<!--            <li><a href="{$url}/Record/{$id|escape:"url"}/SMS" class="sms" onClick="getLightbox('Record', 'SMS', '{$id|escape}', null, '{translate text="Text this"}'); return false;">{translate text="Text this"}</a></li> -->
	    ...
	    {if $user} 
            ...
            {else}
           <li id="saveLink"><span class="notloggedin tooltip fav" title="{translate text='Please login first to use this function!'}">{translate text="Add to favorites"}</span></li>
            {/if}
            {if !empty($addThis)}
        ...
        <div id="tabnav">
            <ul>
              ...
              {if $hasMPGDownLink}
                  <li id="MPGDownLink"{if $tab == 'DownLinkRecords'} class="active"{/if}>
                  <a href="{$url}/Record/{$id|escape:"url"}/DownLinkRecords#tabnav" class="first"><span></span>
                  {translate text='DownTitles'}</a>
                  </li>
              {/if}
  ...
  <div class="yui-b">
	...
	{* show only if edition has title! *}
	{if !empty($similar.title)}
        <li>
          ...
          {/if}
          <a href="{$url}/Record/{$similar.id|escape:"url"}">{if is_array($similar.title)}{$similar.title.0|regex_replace:"/\/$/"|escape}{else}{$similar.title|regex_replace:"/\/$/"|escape}{/if}</a>
          </span>
	...
	{if !empty($edition.title_full)}
        <li>
          ...
          {/if}
          <a href="{$url}/Record/{$edition.id|escape:"url"}">{if is_array($edition.title_full)}{$edition.title_full[0]|escape}{else}{$edition.title_full|escape}{/if}</a>
          </span>
