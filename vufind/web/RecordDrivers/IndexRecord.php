...
class IndexRecord implements RecordInterface
{
    ...
    protected $forbiddenSnippetFields = array(
        ...
        'title_full_unstemmed', 'title_auth', 'title_sub', 'spelling', 'id', 'fullrecord'
    );
...
//        $interface->assign('coreProduct', $this->getProduct());
    ...
    public function getExtendedMetadata()
    {
        ...
        $interface->assign('extendedClassificationShort', $this->getMPGClassificationShort());
        $interface->assign('extendedClassificationLong', $this->getMPGClassificationLong());
	$interface->assign('extendedClassificationJEL', $this->getMPGClassificationJEL());
    ...
    public function getDownLinks()
    {
        ...
        $interface->assign('downLinks', $this->getMPGdownLinkRecords());  
 
        return 'RecordDrivers/Index/showDownLinks.tpl';
    }
    ...
    public function getOpenURL()
    {
        ...
        if (in_array('Book', $formats)) {
            ...
            $format = 'Journal';
        } else if (in_array('E-Journal', $formats)) {
            $format = 'Journal';
       } else if (in_array('Buch', $formats)) {
            $format = 'Book';
       } else if (in_array('E-Buch', $formats)) {
            $format = 'Book';
        } else {
            $format = $formats[0];
        }
...
//        $interface->assign('summCallNo', $this->getCallNumber());
        ...
        $interface->assign('summAuthor', $mainAuthor);
    ...
    {
        if (isset($this->fields['title_short'][0])) {
            return $this->fields['title_short'][0];
        }
    ...
    {
        if (isset($this->fields['title'][0])) {
            return $this->fields['title'][0];
        }
    ...
    protected function getMPGSource()
    {
        return isset($this->fields['sourceStr']) ?
            $this->fields['sourceStr'] : '';
    }
   ...
   protected function getMPGAbrufzeichen()
    ...
    protected function isGBVRecord()
    {
        foreach ($this->fields['institution'] as $inst) {
            if ($inst == 'findex.gbv.de') {
               return true;
            }
        }
        return false;
