...
class SearchObject_Solr extends SearchObject_Base
{
     ...
     *
     * @param array $datefields name of the Solr fields to return facets for
     *
     * @return array an array with the facet values for each index field 
     * @access public
     */

    public function getFullFieldFacets($facetfields = array('publishDate'))
    {
        // buid query string
        $search = $this->searchTerms;
        $query = $this->indexEngine->buildQuery($search);
        ...
        // unliminted facet values for $facetfields
        $facetSet = array();
        $facetSet['limit'] = '-1';
        $facetSet['field'] = $facetfields;
        $facetSet['sort'] = 'index';

        // keep filters except for $datefield filters
        $filterQuery = array();
        $filterList = $this->filterList;
        foreach($facetfields as $facetfield) {
            unset($filterList[$facetfield]);
        }
        ...
        // do search
        $result = $this->indexEngine->search(
            $query,      // Query string
            $this->index,      // DisMax Handler
            $filterQuery,      // Filter query
            0,                 // Starting record
            0,                 // Records per page
            $facetSet,     // Fields to facet on
            null         ,       // Spellcheck query
            null, // Spellcheck dictionary
            '',       // Field to sort on
            $this->fields,     // Fields to return
            $this->method,     // HTTP Request method
            false              // Include errors in response?
        );
        $dateRanges = $result['facet_counts']['facet_fields'];
        // "hash"
        foreach($dateRanges as $key => $dateRange) {
            unset($dateRanges[$key]);
            $dateRanges[$key]['data'] = $dateRange;
        }
        return $dateRanges;
    }
