...
class JSON extends Action
{
     ...
     * @access public
     */
    public function emailLib()
    {
        include_once 'services/Search/EmailLib.php';

        $emailService = new EmailLib();
        $result = $emailService->sendEmail(
            $_REQUEST['url'], $_REQUEST['to'], $_REQUEST['from'],
            $_REQUEST['message']
        );
    ...
    public function getResolverLinks()
    {
        ...
        foreach ($result as $link) {
            ...
            case 'getHolding':
                // RDG: not active (sfx.php = return full SFX menu ($svc.fulltext=no!)
                //$print[] = $link;
                break;
            case 'getWebService':
                // RDG: not active (sfx.php = return full SFX menu ($svc.fulltext=no!)
            ...
            case 'getDOI':
                ...
                $link['coverage'] = '';
            // RDG: check local holdings in SFX (MPG/SFX-EZB-Check!)
            case 'getSelectedFullTxt':
              $electronic[] = $link;
              break;
            case 'getFullTxt':
              $electronic[] = $link;
            default:
              // RDG: not active (sfx.php = full SFX menu ($svc.fulltext=no!)
     ...
     * RDG
     *
     * Get suggestions for given JEL code from ZBW-API
     * Proxy function to avoid JS issues
     * alternatively you can also use JSONP from the API
     *
     * @return void
     * @access public
    ...
    public function getJEL() {
    {
      ...
      $url = 'http://zbw.eu/beta/econ-ws/suggest' .
        '?dataset=jel&query='.$code.'&mode=json';
      ...
      if (PEAR::isError($result)) {
        $this->output("ERROR", JSON::STATUS_OK);
      }
      ...
      if (!PEAR::isError($info)) {
        //          $this->output($info, JSON::STATUS_OK);
        print_r($info);
      }
    }
    }
